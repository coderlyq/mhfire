...
data() {
	...
	beforeCreate(){
		axios.get('http://test.mhfire.cn/mhApi/User/getCode',{
				params: {}
		})
		.then(function(response){
			var decode = atob(response.data.data.base64EnCodeString);
      // 编码转字符串
      // var str = decodeURI(response.data.data.base64EnCodeString);
			console.log(decode);
		})
		.catch(function(error){
				console.log(error);
